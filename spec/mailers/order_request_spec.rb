...
RSpec.describe OrderRequestMailer, type: :mailer do
  # let!(:template) { create :email_template, kind: 'order_request' }
  let!(:template) { EmailTemplate.find_by(kind: 'order_request') }
  ...
  describe 'chat_start' do
    let(:mail) { OrderRequestMailer.notify(order: order) }
