...
RSpec.describe Partners::RecreationsController, type: :request do
  ...
  describe 'GET /new' do
    context 'with valid right' do
      it 'returns http success' do
        get new_partners_recreation_path
        expect(response).to have_http_status(:ok)
      end
    end
  ...
  describe 'POST /create' do
    ...
    context 'when valid parameters' do
      it 'returns 302 status after created' do
        post partners_recreations_path, params: { recreation: attrs }
        expect(response).to have_http_status(:found)
        expect(response).to redirect_to partners_recreation_path(Recreation.last.id)
      end
