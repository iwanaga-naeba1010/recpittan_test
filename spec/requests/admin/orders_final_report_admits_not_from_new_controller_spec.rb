...
RSpec.describe 'Orders::FinalReportAdmitsNotFromNew', type: :request do
  ...
  describe 'POST #create' do
    let(:attrs) { attributes_for(:order, recreation_id: partner.recreations.first.id, user_id: customer.id, start_at: Date.yesterday, end_at: Date.yesterday, is_accepted: true) }
    ...
    context 'with valid parameters' do
      ...
      it 'creates a order and status is final_report_admits_not' do
        post admin_orders_final_report_admits_not_from_news_index_path, params: { orders_final_report_admits_not_from_new: attrs }
        expect(response).to have_http_status(:found)
        expect(Order.last.status).to eq 'final_report_admits_not'
      end
    end
