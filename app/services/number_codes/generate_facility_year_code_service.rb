...
module NumberCodes
  class GenerateFacilityYearCodeService
    attr_reader :registration_year, :code

    def initialize(code, registration_year = Time.zone.today.year.to_s)
    ...
    def formatted_code(numbering_value)
      "P#{registration_year}-#{code}-#{format_number(numbering_value)}"
    end
